...
void Stage::Load() {
  ...
  container_ =
      scene->AddHook(new soil::stage::scene::volume::Container(quadTree));
  // Frustum culling uses the data of the container for culling based on culling
  // scene->AddComponentFeature(new
  // soil::stage::scene::volume::FrustumCulling(viewer, quadTree));
...
void Stage::initCarrots(soil::stage::scene::Scene* scene, byte textureUnit) {
  ...
  for (auto i = 0; i < 4; ++i) {
    ...
    initRotation += 45.F;
    // Add volume
    shapeNode->AddComponent(new soil::stage::scene::component::BoundingVolume(
        new soil::world::volume::AABB(glm::vec3(1, 1, 0)),
        soil::stage::scene::component::BoundingVolume::ContactType::
            StaticObject));
  }
}
...
void Stage::Handle(const soil::WindowEvent& event) {
  ...
  if (printStatistics_ &&
    ...
    PLOG_DEBUG << "FPS: " << std::to_string(stats.FPS)
               << " Draws: " << std::to_string(stats.DrawCount)
