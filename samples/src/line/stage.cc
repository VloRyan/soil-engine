...
namespace soil_samples::line {
    ...
    void Stage::OnLoad() {
        ...
        auto* mesh = GetResources().GetMesh({.Type = soil::video::mesh::Prefab::Type::Line});
        auto* instancing = scene->AddHook(new soil::stage::scene::render::Instancing(scene->GetRenderContainer()));
        instancing->AddRenderBatch(LineInstance::BATCH_NAME,
                                   {.Mesh = mesh, .Shader = shader, .VertexAttribDescriptors = LineInstance::ATTRIBS});
    ...
    void Stage::Update() {
        /* if (lines_[0] != nullptr) {
             ...
             for (auto i = 0; i < MAX_LINES; ++i) {
                 lines_[i]->SetStartPoint(glm::vec3(inner * cos(static_cast<float>(i + offset_)),
                                                    inner * sin(static_cast<float>(i + offset_)), -0.1));
                 lines_[i]->SetEndPoint(glm::vec3(outer * cos(static_cast<float>(i + offset_)),
                                                  outer * sin(static_cast<float>(i + offset_)), -0.1));
             }
             if (offset_ >= 360) {
                 offset_ = 0;
             }
    ...
    void Stage::initLines(soil::stage::scene::Scene* scene) {
        ...
        for (auto i = 0; i < MAX_LINES; ++i) {
            auto* node = scene->AddChild(new soil::stage::scene::Node(soil::stage::scene::Node::Type::Transform));
            lines_[i] = node->AddComponent(new LineInstance(LineInstance::BATCH_NAME, //
