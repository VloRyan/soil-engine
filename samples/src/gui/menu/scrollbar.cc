...
void Scrollbar::SetOnValueChange(
    const std::function<void(float)>& onValueChange) {
...
float Scrollbar::GetMinValue() const { return minValue_; }
...
float Scrollbar::GetMaxValue() const { return maxValue_; }
...
float Scrollbar::GetIncrement() const { return increment_; }

void Scrollbar::SetIncrement(const float increment) { increment_ = increment; }
