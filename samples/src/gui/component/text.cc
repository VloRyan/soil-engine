...
namespace soil_samples::gui::component {
    Text::Text(const std::string& prefab, const std::string& text) : AbstractText(prefab, text) {}
        ...
        for (auto& line : GetLines()) {
            ...
            for (auto& word : line.Words) {
                for (auto i = 0; i < word.Characters.size(); ++i) {
                    ...
                    const glm::vec2 halfSize = (glm::vec2(character.Size) * glm::vec2(0.5F)) * GetCharacterSize();
                    const glm::vec2 fontOffset = glm::vec2(character.Offset) * GetCharacterSize();

                    const glm::vec2 centerPosition(fontOffset.x + halfSize.x,
                    ...
                    shader->SetUniform("uPositionWorldspace",
                                       glm::vec3(localPosition.x + parentPos.x, localPosition.y + parentPos.y,
                                                 parentPos.z + GetPositionOffset().z));


                    shader->SetUniform("uSize", glm::vec2(character.Size) * GetCharacterSize());
                    const auto texCoords =
                        glm::vec2(character.TextureCoord.x, GetFont()->ImageSize.y - character.TextureCoord.y) /
                        glm::vec2(GetFont()->ImageSize);
                    const auto relSize = glm::vec2(character.Size) / glm::vec2(GetFont()->ImageSize);
                    shader->SetUniform("uTexCoords", texCoords);
                    shader->SetUniform("uTexCoordsMax", glm::vec2(texCoords.x + relSize.x, texCoords.y - relSize.y));
                    ...
                    const auto advance = static_cast<float>(character.XAdvance) * GetCharacterSize();
    ...
    float Text::DistanceTo(const glm::vec3& point) {
        return glm::distance(GetParent()->GetWorldPosition().z + GetPositionOffset().z, point.z);
    }
