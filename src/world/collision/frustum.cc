...
namespace world::collision
{
    ...
    bool Frustum::IsVisible(const std::vector<glm::vec4> &point, const int count) const
    {
        for (auto planeIndex = 0; planeIndex < PlanesCount; ++planeIndex)
        {
            ...
            {
                float dist = glm::dot(planes_[planeIndex], point[pointIndex]);
                /*if (dist < 0.0f) {
                  ++out;
                } else {
                  ++in;
    ...
    bool Frustum::IsVisible(const glm::vec3 &point, float ratio) const
    {
        const auto plane = glm::vec4(point, 1.0F);
        for (auto i = 0; i < 6; ++i)
        {
    ...
    glm::vec3 Frustum::GetPoint(Points point) const
    {
        ...
        default:
            throw engine::Exception("unknown Point");
        }
    }
