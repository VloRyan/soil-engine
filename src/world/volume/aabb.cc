...
AABB::AABB(const glm::vec3 dimension)
    : points_(), worldPoints_(), minPoint_(0.F), maxPoint_(0.F) {
...
IntersectionResult AABB::IntersectsRayXZ(const glm::vec3& start,
                                         const glm::vec3& dir) const {
  return IntersectsRay({start.x, 0.F, start.z}, {dir.x, 0.F, dir.z});
}

bool AABB::IntersectsCircle(const glm::vec2& circleCenter,
  ...
  /*const auto halfDim = glm::vec2(maxPoint_.x - minPoint_.x, maxPoint_.z -
  minPoint_.z) * glm::vec2(0.5F); const auto center = glm::vec2(minPoint_.x,
  minPoint_.z) + halfDim; const auto dist = glm::abs(circleCenter - center); if
  (dist.x <= radius && dist.y <= radius) { return true;
  }
