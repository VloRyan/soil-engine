...
bool Frustum::IsVisible(const std::vector<glm::vec3> &point,
...
bool Frustum::IsVisible(const glm::vec3 &point) const {
  ...
  for (auto planeIndex = 0; planeIndex < static_cast<int>(Planes::COUNT);
       ++planeIndex) {
...
bool Frustum::IntersectBox(const glm::vec3 &min, const glm::vec3 &max) const {
  if (!IsVisible(Planes::Left, min, max) ||
      !IsVisible(Planes::Right, min, max) ||
      !IsVisible(Planes::Top, min, max) ||
      !IsVisible(Planes::Bottom, min, max) ||
...
bool Frustum::IsVisible(Planes plane, const glm::vec3 &min,
...
glm::vec4 Frustum::normalize(const glm::vec4 plane) {
  ...
  return plane /
         length;  // Notice the negation. I don't know why I needed that!!
}

glm::vec3 Frustum::intersectionPoint(glm::vec4 planeA, glm::vec4 planeB,
                                     glm::vec4 planeC) {
  glm::vec3 v1;
  glm::vec3 v2;
  glm::vec3 v3;
