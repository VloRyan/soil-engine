...
Mesh::Mesh(int id, std::string name, const ushort *indices,
           const ushort indexCount, const byte *data, const size_t dataSize,
           const size_t vertexSize, const vertex::VertexType targetType)
    : id_(id), name_(std::move(name)),
      indexType_(vertex::IndexType::TYPE_USHORT), indexSize_(0),
      indices_(nullptr), indexCount_(0),
      vertexDataType_(vertex::VertexType::VERT_2D), vertexData_(nullptr),
      vertexCount_(0), vertexDataSize_(0), materialIndex_(0), minCoords_(0.0F),
      maxCoords_(0.0F) {
  SetIndices(indices, indexCount);
  SetData(data, dataSize, vertexSize, targetType);
}

Mesh::Mesh(int id, std::string name, const uint *indices, const uint indexCount,
           const byte *data, const size_t dataSize, const size_t vertexSize,
...
uint Mesh::GetIndex(const uint idx) const {
  ...
  case vertex::IndexType::TYPE_UINT:
    return ((uint *)indices_)[idx];
  case vertex::IndexType::TYPE_USHORT:
    return ((ushort *)indices_)[idx];
  default:
    throw engine::Exception("unknown index type");
  }
}
...
void Mesh::SetData(const byte *data, const size_t dataSize,
  ...
  case vertex::VertexType::VERT_3D:
    ...
    posComponents = 3;
    break;
  case vertex::VertexType::TEXTURED:
    targetTypeSize = sizeof(vertex::VertexTextured);
    posComponents = 3;
    break;
  case vertex::VertexType::BONE:
    targetTypeSize = sizeof(vertex::BoneVertex);
    posComponents = 3;
    break;
  case vertex::VertexType::NORMAL:
    targetTypeSize = sizeof(vertex::NormalMappingVertex);
    posComponents = 3;
    break;
  case vertex::VertexType::NORMAL_BONE:
    targetTypeSize = sizeof(vertex::NormalMappingBoneVertex);
    posComponents = 3;
    break;
  }
...
Mesh *Mesh::New(const int id, std::string name, const PrimitiveType type) {
  ...
  case PrimitiveType::Line: {
    ...
    return new Mesh(id, std::move(name), indices, 2, (const byte *)vertices,
                    2 * sizeof(float), sizeof(float),
                    vertex::VertexType::VERT_2D);
  }
  case PrimitiveType::Quad: {
    ...
    return new Mesh(id, std::move(name), indices, 6, (const byte *)vertices,
  ...
  case PrimitiveType::Box: {
    ...
    return new Mesh(id, std::move(name), indices, 36, (const byte *)boxVertices,
                    24 * sizeof(float), 3 * sizeof(float),
                    vertex::VertexType::VERT_3D);
  }
  default:
    throw engine::Exception("Unknown type");
  }
}
