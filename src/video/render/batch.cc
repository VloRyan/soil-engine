...
Batch::Batch(const mesh::Mesh &mesh, shader::InstanceShader *shader,
             const bool opaque, const int preserveInstances)
  ...
  buffer::Object *instanceVbo =
      new buffer::Vbo(buffer::Object::UsageType::Dynamic);
  instanceVbo->SetData(nullptr, bufferSize);

  size_t offset = 0;
  for (auto [Elements, Type] : shader->GetInstanceVertexDescriptors()) {
    offset = vao_->AddAttributePointer(instanceVbo, Type, Elements,
                                       static_cast<GLsizei>(instanceSize),
                                       offset, true);
  }
  vao_->Create(mesh.GetIndices(), mesh.GetIndexSize());
  if (opaque) {
    instanceBuffer_ = new instance::SortedBuffer(
        instanceVbo, instanceSize,
        instance::SortedBuffer::SortType::FrontToBack);
  } else {
    instanceBuffer_ = new instance::SortedBuffer(
        instanceVbo, instanceSize,
        instance::SortedBuffer::SortType::BackToFront);
  }
}
...
void Batch::AddNewInstance(instance::Instance *instance) const {
  instanceBuffer_->AddInstance(instance);
}
...
void Batch::Render() {
  ...
  shader::Shader::DrawElementsInstanced(
      static_cast<int>(shader_->GetDrawMode()), indexCount_, indexType_, amount,
      0);
}

bool Batch::IsOpaque() const { return opaque_; }

std::string Batch::MakeKey(const mesh::Mesh &mesh,
                           const shader::InstanceShader *shader,
                           const bool opaque) {
  return std::to_string(mesh.GetId()) + "_" + shader->GetName() + "_" +
