...
namespace soil::video::render {
    State::State() :
        depthTest_(false), depthFunc_(DepthFunc::Less), stencilTest_(false), scissorTest_(false), blend_(false),
        changes_(0), maxImageUnits_(0), uboMatrices_(nullptr), framebuffer_(nullptr), viewPort_({}), scissor_({}) {}
    ...
    bool State::IsScissorTest() const {
        return scissorTest_;
    }
    ...
    void State::SetScissor(const Rect& rect) {
        ...
        scissor_ = rect;
        glScissor(scissor_.LowerLeftPosition.x, scissor_.LowerLeftPosition.y, scissor_.Size.x, scissor_.Size.y);
#ifdef DEBUG
        changes_++;
#endif
    }

    Rect State::GetScissor() const {
        return scissor_;
    }
    ...
    void State::SetViewPort(const Rect& rect) {
        ...
        viewPort_ = rect;
        glViewport(rect.LowerLeftPosition.x, rect.LowerLeftPosition.y, rect.Size.x, rect.Size.y);
#ifdef DEBUG
        changes_++;
#endif
    }
    ...
    void State::SetTexture(const GLenum target, const byte textureUnit, texture::Texture& texture) {
        if (textureUnit >= maxImageUnits_) {
            throw std::runtime_error("State::SetTexture: textureUnit out of range, max. image units = " +
    ...
    int State::GetMaxImageUnits() const {
        return maxImageUnits_;
    }
