...
BindBuffer::BindBuffer(const std::string& id, buffer::FrameBuffer* frameBuffer,
                       const bool clearBuffer)
    : Base(id, Type::Prepare),
      frameBuffer_(frameBuffer),
      clearBuffer_(clearBuffer) {}

void BindBuffer::Process(Context& context) {
  context.State->SetFramebuffer(GetFrameBuffer());
  if (const buffer::FrameBuffer* framebuffer = GetFrameBuffer();
      framebuffer != nullptr) {
...
buffer::FrameBuffer* BindBuffer::GetFrameBuffer() const { return frameBuffer_; }

bool BindBuffer::IsClearBuffer() const { return clearBuffer_; }
