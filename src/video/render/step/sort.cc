...
namespace soil::video::render::step {
    ...
    Sort::Order Sort::GetOrder() const {
        return order_;
    }

    void Sort::sortFrontToBack(const glm::vec3& refPoint, std::vector<RenderableObject*>& renderables) {
    ...
    void Sort::sortBackToFront(const glm::vec3& refPoint, std::vector<RenderableObject*>& renderables) {
        std::ranges::sort(renderables,
                          std::function<bool(RenderableObject * a, RenderableObject * b)>(
                              [refPoint](RenderableObject* a, RenderableObject* b) {
                                  if (a->IsSortable() && b->IsSortable()) {
