...
VertexArray::VertexArray(const mesh::Data& mesh)
    : vao_(nullptr), vbo_(nullptr), drawMode_(mesh.GetDrawMode()) {
  vao_ = new vertex::Vao();
  ...
  for (const auto [Elements, Type] : mesh.GetVertexAttribs()) {
    offset =
        vao_->AddAttributePointer(vbo_, Type, Elements, vertexSize, offset);
  }
...
vertex::Vao* VertexArray::GetVao() const { return vao_; }
