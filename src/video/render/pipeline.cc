...
void Pipeline::find(std::vector<step::Base*>::iterator& itr,
...
std::vector<step::Base*>& Pipeline::GetSteps() { return steps_; }

void Pipeline::SetName(std::string Name) { name_ = std::move(Name); }

std::string Pipeline::GetName() const { return name_; }

step::Context& Pipeline::GetContext() { return context_; }

Pipeline* Pipeline::NewForwardRenderingPipeline(Container* container,
                                                bool depthTest,
