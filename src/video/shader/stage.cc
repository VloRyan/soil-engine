...
Stage::Stage(const uint id, const uint type)
    : id_(id), programId_(0), type_(type) {}
...
size_t Stage::replaceLine(const std::string& line,
  ...
  if (line.starts_with("//#include ")) {
    ...
    std::string includeFile =
        currentDir + "includes/" + includeFilename + ".glsl";
    std::ifstream in(includeFile);
    if (!in.is_open()) {
      std::string message =
          "Impossible to open the include file '" + includeFilename + "'";
...
Stage* Stage::load(std::string fileName, uint type) {
  ...
  if (!in.is_open()) {
    std::string message =
        "Impossible to open the shader file '" + fileName + "'";
    throw std::runtime_error(message);
  }
  ...
  if (stage == nullptr) {
    std::string message =
        "Impossible to compile the shader file '" + fileName + "'";
    throw std::runtime_error(message);
  }
  return stage;
}
