...
AttributePointer::AttributePointer(buffer::Object *vbo, const DataType dataType,
    ...
    : vbo_(vbo), dataType_(dataType), elementSize_(elementSize),
      elementStride_(elementStride), divisor_(divisor), offset_(offset),
      normalize_(false) {}
...
void AttributePointer::Set(const uint index) const {
  ...
  if (isIntegerType) {
    ...
    glVertexAttribPointer(index, this->GetElementSize(), glType, normalize,
                          this->GetElementStride(), (void *)this->GetOffset());
  }
...
uint AttributePointer::GetGLDataType() const {
  ...
  default:
    throw engine::Exception(
        "Unknown data type: " +
        std::to_string(static_cast<uint>(this->GetDataType())));
  }
}
...
size_t AttributePointer::GetSizeOfDataType(const DataType type) {
  ...
  case UInt64:
    ...
    ;
  default:
    throw engine::Exception("Unknown data type: " + std::to_string((int)type));
  }
}
