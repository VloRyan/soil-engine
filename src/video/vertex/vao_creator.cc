...
namespace soil::video::vertex {
    Vao *VaoCreator::Build(const VertexType type, const byte *data, const gl_size_t dataSize, const int vertexCount) {
        ...
        case VERT_2D:
            // Position
            vao->AddAttributePointer(vbo, AttributePointer::DataType::Float, 2, vertexSize, 0, false);
            break;
        case VERT_3D:
            // Position
            vao->AddAttributePointer(vbo, AttributePointer::DataType::Float, 3, vertexSize, 0, false);
            break;
        default:
