...
namespace soil::video::buffer {
    Ebo::Ebo(const vertex::IndexType indexType, const UsageType usage, const AccessType access) :
        Object(Types::ElementArray, usage, access), indexType_(indexType), indexCount_(0) {
        Object::create();
    }

    Ebo::~Ebo() = default;

    vertex::IndexType Ebo::GetIndexType() const {
        return indexType_;
    }
