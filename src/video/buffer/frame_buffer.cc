...
namespace video::buffer {
    ...
    void FrameBuffer::Resize(const glm::ivec2 newSize) {
        if (const bool sizeChanged = glm::any(glm::notEqual(size_, newSize)) && newSize.x > 0; !sizeChanged) {
            return;
        }

        // copy values
        const DepthBufferType depthBufferType = depthType_;
        std::vector<texture::Parameter> texParameterBuffer(colorBufferParameter_.size());
        ...
        Unload();
        size_ = newSize;
        for (auto i = 0; i < texParameterBuffer.size(); i++) {
            CreateColorTexture(texParameterBuffer[i]);
        }
        CreateDepthBuffer(depthBufferType);

        if (m_DepthTexture != nullptr) {
            bool onlyDepth = texParameterBuffer.size() == 0;
            CreateDepthTexture(onlyDepth);
        }
    }

    uint FrameBuffer::CreateDepthBuffer(const DepthBufferType depthType, const int samples) {
        if (id_ == 0U) {
            return 0;
        }
        ...
        if (depthBufferId_ != 0U) {
            throw engine::Exception("depth buffer exists");
        }
        ...
        if (samples > 1) {
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, samples, internalFormat, size_.x, size_.y);
        } else {
        ...
        // attach buffers
        Bind();
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, attachment, GL_RENDERBUFFER, depthBufferId_);
        CheckState();
    ...
    texture::Texture* FrameBuffer::CreateDepthTexture(const bool onlyDepth) {
        ...
        if (depthBufferId_ != 0U) {
            throw engine::Exception("Depthbuffer exists");
        }
        ...
        m_DepthTexture = texture::Manager::GenerateTexture2D(data, "depthTexture_" + std::to_string(id_), parameter);
        //loadTexture2D(m_DepthTexture, &data, parameter);
    ...
    uint FrameBuffer::CreateColorBuffer(const int samples) {
        ...
        if (colorBufferId_ != 0U) {
            throw engine::Exception("colorbuffer already Exists");
        }
        ...
        // attach buffers
        Bind();
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, attachment, GL_RENDERBUFFER, colorBufferId_);
        CheckState();
    ...
    texture::Texture* FrameBuffer::CreateColorTexture(const texture::Parameter &parameter) {
        if (id_ == 0U) {
            return nullptr;
        }
        const texture::Data data(nullptr, size_, GL_RGB, GL_UNSIGNED_BYTE);
        auto *texture = texture::Manager::GenerateTexture2D(data, "colorTexture_" + std::to_string(id_), parameter);
        Bind();
        ...
        if (colorBufferVector_.size() > 1) {
            ...
            glDrawBuffers(colorBufferVector_.size(), attachments.data());
        } else {
    ...
    texture::Texture* FrameBuffer::GetColorBufferTexture(const int index) const {
        if (index >= colorBufferVector_.size()) {
            throw engine::Exception(
                "Index out of bounds: index(" + std::to_string(index) + ") size(" + std::to_string(
                    colorBufferVector_.size())
                + ")");
        }
    ...
    void FrameBuffer::CheckState() {
        ...
        switch (state) {
            case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
                throw engine::Exception("INCOMPLETE_ATTACHMENT");
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
                throw engine::Exception("INCOMPLETE_MISSING_ATTACHMENT");
            case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
                throw engine::Exception("INCOMPLETE_DRAW_BUFFER");
            case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
                throw engine::Exception("INCOMPLETE_READ_BUFFER");
            case GL_FRAMEBUFFER_UNSUPPORTED:
                throw engine::Exception("UNSUPPORTED");
            default:
                throw engine::Exception("UNKNOWN");
        }
    }
    ...
    void FrameBuffer::BlitTo(const FrameBuffer &otherFbo) const {
        ...
        glBlitFramebuffer(0, 0,
                          GetSize().x, GetSize().y,
                          0, 0,
                          otherFbo.GetSize().x, otherFbo.GetSize().y,
