...
namespace soil::video::buffer {
    FrameBuffer::FrameBuffer(const glm::ivec2 size) :
        id_(0), colorBufferId_(0), depthBufferId_(0), stencilBufferId_(0), m_DepthTexture(nullptr),
        colorBufferParameter_(0), size_(size), depthType_(DepthBufferType::None), clearBufferBits_(0) {
    ...
    texture::Texture* FrameBuffer::CreateColorTexture(const texture::Parameter& parameter) {
        ...
        Bind();
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + colorBufferVector_.size(), GL_TEXTURE_2D,
                               texture->GetId(), 0);
        CheckState();
        ...
        if (colorBufferVector_.size() > 1) {
            ...
            glDrawBuffers(static_cast<int>(colorBufferVector_.size()), attachments.data());
        } else {
    ...
    void FrameBuffer::BlitTo(const FrameBuffer& otherFbo) const {
        ...
        glBlitFramebuffer(0, 0, GetSize().x, GetSize().y, 0, 0, otherFbo.GetSize().x, otherFbo.GetSize().y,
