...
Texture* Manager::GetTexture2D(const std::string& fileName,
                               const Parameter& parameter) {
  // Check if texture is loaded
  for (auto entry : texturesCache_) {
    const bool sameType = entry.texture->GetType() == Texture::Type::Texture2D;
    const bool sameName = !entry.texture->GetName().empty() &&
                          entry.texture->GetName() == fileName;
...
Texture* Manager::GetTextureArray2D(const std::string& fileName,
  ...
  // Check if texture is loaded
  for (auto entry : texturesCache_) {
    if (const bool sameType =
            entry.texture->GetType() == Texture::Type::TextureArray2D;
        sameType && !entry.texture->GetName().empty() &&
        entry.texture->GetName() == fileName) {
      entry.useCount++;
      return entry.texture;
    }
  ...
  auto* texture =
      GenerateTextureArray2D(*data, fileName, tilesPerDim, parameter);
  delete data;
  const auto entry = texturesCache_.emplace_back(texture);
  return entry.texture;
}

Texture* Manager::GenerateTextureArray2D(const Data& data,
  ...
  for (int iy = 0; iy < tilesPerDim; ++iy) {
    for (int ix = 0; ix < tilesPerDim; ++ix) {
      ...
      for (int row = 0; row < tileSize; ++row) {
        std::copy_n(ptr + row * rowLen, tileSizeX,
                    tile.begin() + row * tileSizeX);
      }
      const int tileIndex = tilesCount - tilesPerDim - iy * tilesPerDim + ix;
      glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, tileIndex, tileSize,
                      tileSize, 1, data.Format, data.Type, tile.data());
  ...
  glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER,
                  static_cast<GLint>(parameter.MinFilter));
  glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER,
                  static_cast<GLint>(parameter.MagFilter));

  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  static_cast<GLint>(parameter.Wrap));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                  static_cast<GLint>(parameter.Wrap));
  logGLError("Error before glTexParameteri");
  glBindTexture(GL_TEXTURE_2D_ARRAY, 0);
  return new Texture(id, name, data.Size, Texture::Type::TextureArray2D,
                     parameter.Format);
}
...
Data* Manager::loadData(const std::string& fileName) {
  ...
  ILuint ilImgId = 0;
  const bool isWithPath = fileName.find('\\') != std::string::npos ||
  ...
  auto* const data = new Data(
      ilGetData(),  // will copy data
...
Texture* Manager::GetCubeMap(const std::string& name, const std::string& ext,
  ...
  const auto entry =
      texturesCache_.emplace_back(new Texture(openGlId, name, glm::uvec2(0)));
  return entry.texture;
}

uint Manager::loadCubeMap(const std::string& name, const std::string& ext,
                          const Texture::Format internalFormat) {
...
void Manager::loadCubeMapFace(const std::string& texturePathFile,
                              const GLenum target,
                              const Texture::Format internalFormat) {
  ILuint ilImgId = 0;
  // Generate the main image name to Use.
  ...
  glTexImage2D(target, 0, static_cast<GLint>(internalFormat), width, height, 0,
               format, type, ilGetData());
  logGLError("Error after glTexImage2D");
  ...
  if (const GLenum glError = glGetError(); glError != GL_NO_ERROR) {
    switch (glError) {
      ...
      case GL_INVALID_OPERATION:
        ...
        break;
      default:
        PLOG_ERROR << text << ": Unrecognised GLenum("
...
TextureCacheEntry::TextureCacheEntry(Texture* texture)
    : useCount(1), texture(texture) {}
}  // namespace soil::video::texture
