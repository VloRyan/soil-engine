...
Event::Event(const Origin origin, const Cause cause)
    : event::Event(Type::Input), cause_(cause), origin_(origin),
      key_(Keys::Unknown), mouseButton_(MouseButton::Left),
      state_(State::Release), character_(' '), wheelOffset_(glm::vec2(0)) {}
...
Event Event::MouseButtonEvent(const MouseButton button, const State state,
                              const glm::vec2 pos) {
