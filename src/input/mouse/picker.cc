...
namespace soil::input::mouse {
    ...
    glm::vec2 Picker::getNormalisedDeviceCoordinates(const glm::uvec2 &mousePos, const glm::uvec2 &windowSize) {
        glm::vec2 nds;
        nds.x = 2.0F * static_cast<float>(mousePos.x) / static_cast<float>(windowSize.x) - 1.0F;
        nds.y = 1.0F - 2.0F * static_cast<float>(mousePos.y) / static_cast<float>(windowSize.y);
        return nds;
    }
