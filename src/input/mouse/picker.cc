...
glm::vec3 Picker::CalculateRay(const glm::uvec2 &mousePos,
...
glm::vec2 Picker::getNormalisedDeviceCoordinates(const glm::uvec2 &mousePos,
                                                 const glm::uvec2 &windowSize) {
  glm::vec2 nds;
  nds.x =
      2.0F * static_cast<float>(mousePos.x) / static_cast<float>(windowSize.x) -
      1.0F;
  nds.y = 1.0F - 2.0F * static_cast<float>(mousePos.y) /
                     static_cast<float>(windowSize.y);
  return nds;
}

glm::vec4 Picker::toEyeCoords(const glm::vec4 &clipCoords,
...
glm::vec3 Picker::toWorldCoords(const glm::vec4 &eyeCoords,
