...
Manager::Manager() : window_(nullptr), eventQueue_(&eventQueueBack1_) {}
...
void Manager::Init(engine::Window *window) {
  ...
  glfwSetKeyCallback(window_->GetGLFWWindow(),
                     [](GLFWwindow *window, const int key, const int scancode,
                       ...
                       instance->eventQueue_->push_back(
                           Event::KeyChangedEvent(getKey(key), state));
                       mutex_.unlock();
                     });
  glfwSetCharCallback(
      ...
      [](GLFWwindow *window, const unsigned int codepoint) {
        mutex_.lock();
        instance->eventQueue_->push_back(
            Event::CharacterEnteredEvent(static_cast<char>(codepoint)));
        mutex_.unlock();
      });
  glfwSetMouseButtonCallback(
      window_->GetGLFWWindow(), [](GLFWwindow *window, const int button,
                                   const int action, const int mods) {
        ...
        mutex_.lock();
        instance->eventQueue_->push_back(
            Event::MouseButtonEvent(mouseButton, state, glm::vec2(x, y)));
        mutex_.unlock();
      });
  glfwSetScrollCallback(
      ...
      [](GLFWwindow *window, const double xoffset, const double yoffset) {
        mutex_.lock();
        instance->eventQueue_->push_back(
            Event::MouseWheelEvent(glm::vec2(xoffset, yoffset)));
        mutex_.unlock();
      });
}
...
void Manager::Update() {
  ...
  mutex_.unlock();
  // eventQueueBackBuffer->push_back(Event::MousePositionEvent(window_->GetMouseCursorPos()));
  /*std::string tmpCharBuffer = m_CharBuffer;
  m_CharBuffer.Clear();
  for (std::string::size_type i = 0; i < tmpCharBuffer.size_(); i++) {
    CharacterEnteredEvent charEntered(tmpCharBuffer.at(i));
    fire(charEntered);
  }*/
  ...
  /*if (m_CursorChanged) {
    boolean isDefaultCursor = pos_ == nullptr || pos_->GetType() ==
  Cursor::Type::DEFAULT; if (isDefaultCursor) { glfwSetCursor(m_CurrentWindow,
  nullptr); } else { GLFWcursor* glfwCursor = getGLFWCursor(pos_->GetType());
      glfwSetCursor(m_CurrentWindow, glfwCursor);
    }
  }*/
}
...
void Manager::processEvents(EventQueue *queue) {
  ...
  for (const Keys pressedKey : pressedKeys) {
    ...
    if (!isReleasedInThisFrame) {
      eventQueue_->push_back(
          Event::KeyChangedEvent(pressedKey, Event::State::Press));
    }
  }
  for (const MouseButton pressedButton : pressedButtons) {
    ...
    if (!isReleasedInThisFrame) {
      ...
      eventQueue_->push_back(Event::MouseButtonEvent(
          pressedButton, Event::State::Press, glm::vec2(x, y)));
    }
  }
}
