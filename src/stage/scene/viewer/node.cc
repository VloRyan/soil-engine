...
namespace stage::scene::viewer {
    Node::Node(scene::Node *parent)
        : scene::Node(nullptr, Type::Viewer), pipeline_(nullptr), viewChanged_(true),
    ...
    video::render::Pipeline* Node::GetPipeline() const {
        return pipeline_;
    }
    ...
    void Node::RemoveRenderable(component::RenderableInstance *renderable) {
        ...
        //const auto batchKey = video::render::Batch::MakeKey(*mesh, shader, renderable->IsOpaque());
        const auto itr = batchMap_.find(batchKey);
        if (itr == batchMap_.end()) {
            return;
        }
    ...
    void Node::SetQuadTree(space::QuadTree *quadTree) {
        quadTree_ = quadTree;
    }
    ...
    void Node::updateVisibilityOnTreeNode(const int index) {
        ...
        std::array points{
            glm::vec3(treeNode->Min.x, 0.0F, treeNode->Min.y),
            glm::vec3(treeNode->Max.x, 0.0F, treeNode->Max.y),
            glm::vec3(treeNode->Min.x, 0.0F, treeNode->Max.y),
            glm::vec3(treeNode->Max.x, 0.0F, treeNode->Min.y)
        };
        ...
        if (nodesVisibility_[index] != isVisible) {
            for (const auto *volume: treeNode->Contained) {
                ...
                for (auto renderables = node->GetComponents(
                         component::Component::Type::Renderable);
                     auto *renderable: renderables) {
                    auto *instance = dynamic_cast<component::RenderableInstance *>
                            (renderable);
    ...
    video::render::StateDef Node::GetStateDef() const {
        return stateDef_;
    }

    void Node::SetStateDef(const video::render::StateDef &stateDef) {
        stateDef_ = stateDef;
    }

    int Node::GetUboMatricesBindingTarget() const {
