...
Ortho::Ortho(const glm::ivec2 resolution)
    : projection_(glm::mat4(1.F)),
      view_(glm::lookAt(glm::vec3(0.F), glm::vec3(0.0F, 0.0F, -1.0F),
...
void Ortho::Update() { Node::Update(); }

void Ortho::UpdateProjection(const glm::ivec2& size) {
  switch (orthoType_) {
    case OrthoType::Full:
      projection_ = glm::ortho(0.F, static_cast<float>(size.x), 0.F,
                               static_cast<float>(size.y), nearZ_, farZ_);
      break;
    case OrthoType::OrthoHeight:
      ...
      projection_ = glm::ortho(-halfWidth, halfWidth, -halfHeight, halfHeight,
                               nearZ_, farZ_);
      break;
  }
  frustum_->SetProjectionView(GetProjectionMatrix() * GetViewMatrix());
...
void Ortho::UpdateDirty() {
  ...
  direction_ =
      glm::vec3(invTransform[2]) * glm::vec3(-1.F);  // direction is inverse
  frustum_->SetProjectionView(GetProjectionMatrix() * GetViewMatrix());
}
}  // namespace soil::stage::scene::viewer
