...
namespace soil::stage::scene::gui::container {
    ...
    const std::vector<Rectangle*>& Base::GetItems() const {
        return items_;
    }

    void Base::Handle(const event::Node& event) {
        if (event.GetChangeType() == event::Node::ChangeType::State && event.GetOrigin()->GetState() == State::Dirty) {
            SetDirty(DirtyImpact::Dependents);
        }
    }

    const glm::ivec2& Base::GetOffset() const {
        return offset_;
    }
    ...
    const glm::ivec2& Base::GetItemsSize() const {
        return itemsSize_;
    }
