...
Instancing::BatchObject* Instancing::createBatch(const std::string& key) {
  ...
  const auto batchDescriptor = video::render::BatchDescriptor{
      .Vao = va->GetVao(),
      .Shader = args.Shader,
...
Instancing::BatchObject::BatchObject(
    const video::render::BatchDescriptor& descriptor)
    : Batch(descriptor) {}
