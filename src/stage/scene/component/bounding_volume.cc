...
bool BoundingVolume::IsInside(const glm::vec3& min,
                              const glm::vec3& max) const {
  return volume_->IsInside(min, max);
}

bool BoundingVolume::IsInsideXZ(const glm::vec3& min,
                                const glm::vec3& max) const {
  return volume_->IsInsideXZ(min, max);
}

bool BoundingVolume::IsInside(const glm::vec2& min,
...
world::volume::IntersectionResult BoundingVolume::IntersectsRay(
    const glm::vec3& start, const glm::vec3& dir) const {
  return volume_->IntersectsRay(start, dir);
}

world::volume::IntersectionResult BoundingVolume::IntersectsRayXZ(
    const glm::vec3& start, const glm::vec3& dir) const {
  return volume_->IntersectsRayXZ(start, dir);
}

bool BoundingVolume::IntersectsCircle(const glm::vec2& circleCenter,
...
glm::vec3 BoundingVolume::GetPosition() const { return volume_->GetPosition(); }
  ...
  if (container_ != nullptr) {
    if (!container_->Remove(this)) {
      throw std::runtime_error(
          "[BoundingVolume::SetContainer]: Failed to remove from container.");
    }
  }
