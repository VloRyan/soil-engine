...
namespace soil::stage::scene {
    ...
    Scene::Scene(Stage *stage) :
        Node(stage->AddScene(this), Type::Scene, {}, ".::SCENE::."), stage_(stage), dirtyNodesPtr_(&dirtyNodes_),
        ...
        case Type::Viewer:
            {
                ...
                if (viewer_.size() == 1) {
                    forEachChild(this, [viewer](const Node *child) {
                        for (const auto renderables = child->GetComponents(component::Component::Type::Renderable);
                             auto *const renderable : renderables) {
                            viewer->AddRenderable(dynamic_cast<component::RenderableInstance *>(renderable));
                        }
                    });
                }
                break;
