...
QuadTree::QuadTree(const float size, const int maxLevel,
                   const float minChildSize, IDebugDrawer *debugDrawer)
...
void QuadTree::Insert(const component::BoundingVolume *volume) {
  insert(volume, 0);
}

bool QuadTree::insert(const component::BoundingVolume *volume,
                      const int index) {
...
bool QuadTree::Remove(const component::BoundingVolume *volume) {
  ...
  for (auto itr = nodes_.at(index).Contained.begin();
       itr != nodes_.at(index).Contained.end(); ++itr) {
    if (*itr == volume) {
...
bool QuadTree::remove(const component::BoundingVolume *volume,
  ...
  if (const auto firstChildIndex = nodes_[index].FirstChildIndex;
      firstChildIndex != -1) {
    for (auto i = 0; i < 4; ++i) {
  ...
  for (auto itr = nodes_[index].Contained.begin();
       itr != nodes_[index].Contained.end(); ++itr) {
    if (*itr == volume) {
...
const QuadTree::Node *QuadTree::FindNodeAt(const glm::vec2 point) {
  return findNodeAt(point, 0);
}

std::vector<const QuadTree::Node *>
QuadTree::FindNodesAt(const glm::vec2 point) {
...
void QuadTree::SetDebugDrawer(IDebugDrawer *const debugDrawer) {
  debugDrawer_ = debugDrawer;
}
...
void QuadTree::split(const int index) {
  ...
  for (auto childIdx = 0; childIdx < 4; childIdx++) {
    ...
    nodes_.emplace_back(index, nodes_[index].Level + 1, centerChild,
                        childRadius);
  }
...
void QuadTree::drawDebugLines(const int index, glm::vec2 centerParent,
                              float childWidth) const {
  glm::vec2 centerChild = centerParent;
  ...
  debugDrawer_->Line(
      glm::vec3(centerChild.x, yOffset, centerChild.y - childWidth),
      glm::vec3(centerChild.x, yOffset, centerChild.y + childWidth), color);
  debugDrawer_->Line(
      glm::vec3(centerChild.x - childWidth, yOffset, centerChild.y),
