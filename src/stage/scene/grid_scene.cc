...
GridScene::GridScene(Stage *stage, const float size, const int maxLevel,
                     const float minChildSize)
    : Scene(stage),
      quadTree_(new space::QuadTree(size, maxLevel, minChildSize)) {}
...
void GridScene::ForNodesAt(
    const glm::vec2 pos,
    const std::function<void(const component::BoundingVolume *volume)> &f)
    const {
...
void GridScene::ComponentStateChanged(component::Component *component) {
  ...
  if (component->GetType() == component::Component::Type::BoundingVolume) {
    if (const auto *volume =
            dynamic_cast<component::BoundingVolume *>(component);
        quadTree_->Remove(volume)) {
      quadTree_->Insert(volume);
    }
  }
