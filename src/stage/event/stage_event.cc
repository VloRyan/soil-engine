...
StageEvent::StageEvent(const TriggerType triggerType)
    : Event(Type::Stage),
      ...
      stage_(nullptr),
      previousStage_(nullptr) {}

Stage* StageEvent::GetStage() const { return stage_; }

Stage* StageEvent::GetPreviousStage() const { return previousStage_; }

bool StageEvent::GetStateChangeFlag(const int flag) const {
  return stateChangeFlags_[flag];
}

StageEvent::TriggerType StageEvent::Trigger() const { return triggerType_; }
...
StageEvent StageEvent::MakeStateChanged(Stage* stage,
