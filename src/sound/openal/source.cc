...
namespace soil::sound::openal {
    ...
    void Source::SetPosition(glm::vec3 position) const { alSourcefv(this->id_, AL_POSITION, &position[0]); }
    ...
    void Source::SetPitch(const float pitch) const { alSourcef(this->id_, AL_PITCH, pitch); }
    ...
    void Source::SetGain(const float gain) const { alSourcef(this->id_, AL_GAIN, gain); }
    ...
    void Source::Play() const { alSourcePlay(this->id_); }

    void Source::Pause() const { alSourcePause(this->id_); }

    void Source::Rewind() const { alSourceRewind(this->id_); }

    void Source::Stop() const { alSourceStop(this->id_); }
    ...
    void Source::SetMaxDistance(const float distance) const { alSourcef(id_, AL_MAX_DISTANCE, distance); }
    ...
    void Source::SetReferenceDistance(const float distance) const { alSourcef(id_, AL_REFERENCE_DISTANCE, distance); }
    ...
    void Source::SetRolloffFactor(const float factor) const { alSourcef(id_, AL_ROLLOFF_FACTOR, factor); }
} // namespace soil::sound::openal
