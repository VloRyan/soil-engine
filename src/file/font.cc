...
Font::Font(std::string fileName)
    : fileName_(std::move(fileName)), lineHeight_(0), base_(0), imageSize_(0),
      spaceWidth_(0), padding_(0) {}
...
video::model::Letter *Font::getCharacter(uint id) {
  auto itr = characterMap_.find(id);
  if (itr != characterMap_.end()) {
    return itr->second;
  }
  return nullptr;
}
...
Font *Font::Load(std::string fontFilePath) {
  ...
  std::string extension = util::Files::GetExtension(fontFilePath);
  if (extension.empty()) {
    fontFilePath += ".fnt";
  ...
  if (!isFontFile) {
    throw engine::Exception("No .fnt file found for expression '" +
                            fontFilePath + "'.");
  }
  std::ifstream ifs;
  ...
  while (std::getline(ifs, line)) {
    file::Font::mapValues(line, mappedValues);
    std::string lineType = file::Font::getValue("LINE_TYPE", mappedValues);
    if (lineType == "info") {
      file::Font::loadInfos(fontFile, mappedValues);
    }
    if (lineType == "common") {
      file::Font::loadCommon(fontFile, mappedValues);
    }
    if (lineType == "page") {
      file::Font::loadPage(fontFile, mappedValues);
    }
    if (lineType == "chars") {
      file::Font::loadChars(fontFile, mappedValues);
    }
    if (lineType == "char") {
      file::Font::loadChar(fontFile, mappedValues);
    }
  }
  ifs.close();
...
void Font::mapValues(std::string line,
                     std::unordered_map<std::string, std::string> &map) {
  std::vector<std::string> parts = util::Strings::split(std::move(line), " ");
  std::string key;
  std::string value;
  ...
  for (const std::string &part : parts) {
    ...
    if (const std::string::size_type posOfEqual = part.find('=');
      ...
      if (map.empty()) {
        ...
        throw engine::Exception("Invalid state(Part: " + part + " Key: " + key +
                                " Value: " + value + ")");
      }
    }
    valueStartsWithDQuote = value.starts_with('\"');           // starts with "
    valueEndsWithDQuote = value.at(value.length() - 1) == '"'; // ends with "
    const bool valueComplete = !valueStartsWithDQuote || valueEndsWithDQuote;
    if (valueComplete) {
...
std::string Font::getValue(const std::string &key,
                           std::unordered_map<std::string, std::string> &map) {
...
void Font::loadInfos(Font *fontFile,
                     std::unordered_map<std::string, std::string> &map) {
  const std::string paddingStr = getValue("padding", map);
  const std::vector<std::string> parts = util::Strings::split(paddingStr, ",");
  if (parts.size() != 4) {
    throw engine::Exception("Invalid padding size: " +
                            std::to_string(parts.size()));
  }
  for (uint i = 0U; i < static_cast<uint>(parts.size()); i++) {
    fontFile->padding_[static_cast<glm::vec4::length_type>(i)] =
        std::stof(parts[i]);
  }
}

void Font::loadCommon(Font *fontFile,
                      std::unordered_map<std::string, std::string> &map) {
...
void Font::loadChars(Font *fontFile,
                     std::unordered_map<std::string, std::string> &map) {
  // DO NOTHING
}

void Font::loadPage(Font *fontFile,
                    std::unordered_map<std::string, std::string> &map) {
  std::string texFile = getValue("file", map);
  if (util::Files::GetDirectory(texFile).empty()) {
    std::string fontFileDir =
        util::Files::GetDirectory(fontFile->getFileName());
    texFile = fontFileDir + texFile;
  }
  fontFile->textureFile_ = texFile;
}

void Font::loadChar(Font *fontFile,
                    std::unordered_map<std::string, std::string> &map) {
  // int PAD_TOP = 0;
  // int PAD_LEFT = 1;
  const float factor =
      12.0f / (fontFile->getLineHeight() - fontFile->getBase());
  uint id = static_cast<uint>(getIntValue("id", map));
  const bool isSpace = id == ' ';
  const float xAdvance = (static_cast<float>(getIntValue("xadvance", map)) -
                          fontFile->getHorizontalPadding()) *
                         factor;
  if (isSpace) {
  ...
  const glm::vec2 textureUV(static_cast<float>(getIntValue("x", map)) /
                                static_cast<float>(fontFile->imageSize_),
                            (static_cast<float>(fontFile->imageSize_) -
                             static_cast<float>(getIntValue("y", map))) /
                                static_cast<float>(fontFile->imageSize_));
  ...
  const glm::vec2 textureSize(
      static_cast<float>(width) / static_cast<float>(fontFile->imageSize_),
      static_cast<float>(height) / static_cast<float>(fontFile->imageSize_));
  const glm::vec2 quadSize(width * factor, height * factor);
  const glm::vec2 offset(
      static_cast<float>(getIntValue("xoffset", map)) * factor,
      static_cast<float>(getIntValue("yoffset", map)) * factor);

  auto *character = new video::model::Letter(
      id, textureUV,
      glm::vec2(textureUV.x + textureSize.x, textureUV.y - textureSize.y),
      offset, quadSize, xAdvance);
  std::unordered_map<uint, video::model::Letter *>::value_type pair(id,
                                                                    character);
