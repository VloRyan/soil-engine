...
void Window::Open() {
  ...
  glfwWindow_ = glfwCreateWindow(parameter_.Size.x, parameter_.Size.y,
...
void Window::registerCallbacks() {
  ...
  glfwSetFramebufferSizeCallback(glfwWindow_, [](GLFWwindow *, const int width,
                                                 const int height) {
  ...
  glfwSetWindowIconifyCallback(
      glfwWindow_, [](GLFWwindow *, const int iconified) {
        instance->stateFlags_[static_cast<short>(WindowState::Minimized)] =
            iconified != 0;
      });

  glfwSetWindowFocusCallback(glfwWindow_, [](GLFWwindow *, const int focused) {
    instance->stateFlags_[static_cast<short>(WindowState::Focused)] =
        focused != 0;
  });

  glfwSetWindowCloseCallback(glfwWindow_, [](GLFWwindow *) {
    instance->stateFlags_ = static_cast<short>(WindowState::Closed);
  });
}
