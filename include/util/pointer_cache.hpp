...
class PointerCache final {
 public:
  ...
  explicit PointerCache(const bool holdPointer,
                        const std::function<void(Tp*)> removeHook = nullptr)
  ...
  std::shared_ptr<Tp> cache(Key key, Tp* value) {
    std::unique_lock lock(mutex_);
    _prune();
    // std::shared_ptr<_Tp> sharedPtr(value,
    // std::bind(&PointerCache::removeValue, this, std::placeholders::_1));
  ...
  void remove(Key key) {
    ...
    if (holdPointer_) {
      ...
      for (auto itrHoldCache = holdCache_.begin();
           itrHoldCache != holdCache_.end(); ++itrHoldCache) {
        if (std::shared_ptr<Tp> holdPointer = *itrHoldCache;
            holdPointer.get() == weakPtr.lock().get()) {
