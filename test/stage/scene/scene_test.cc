...
namespace soil::stage::scene {
    ...
    TEST_F(SceneTest, Update) {
        ...
        node->SetDirty(Node::DirtyImpact::Dependents); // this will also update childOfChild
        ...
        child->SetDirty(Node::DirtyImpact::Dependents); // do not update childOfChild again
        scene.Update();
    ...
    TEST_F(SceneTest, AddChild_with_tree_and_destructor) {
        ...
        const auto* comp1 = node->AddComponent(new component::ComponentMock(component::Component::Type::Visual));
        const auto child = node->AddChild(new NodeMock(Node::Type::Game));
        const auto* comp2 = child->AddComponent(new component::ComponentMock(component::Component::Type::Visual));
        const auto childOfChild = child->AddChild(new NodeMock(Node::Type::Game));
        const auto* comp3 =
            childOfChild->AddComponent(new component::ComponentMock(component::Component::Type::Visual));
    ...
    TEST_F(SceneTest, Remove_listener_on_node_remove) {
        ...
        const auto inputEvent = input::Event::MakeKeyChangedEvent(input::Keys::A, input::Event::State::Press);
