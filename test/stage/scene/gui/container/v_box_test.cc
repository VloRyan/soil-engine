...
TEST_F(VBoxTest, Contructor) {
  ...
  EXPECT_EQ(defaultBox.GetItemAlignment(),
            layout::Anchor::HorizontalAlignments::Center);
...
TEST_F(VBoxTest, Layout) {
  ...
  EXPECT_VEC_EQ(child2->GetPosition(),
                glm::vec3(0.F, 50.0F, Rectangle::LAYER_Z_INCREMENT));
  EXPECT_VEC_EQ(child3->GetPosition(),
                glm::vec3(0.F, -50.0F, Rectangle::LAYER_Z_INCREMENT));

  box.SetItemAlignment(layout::Anchor::HorizontalAlignments::Left);
  box.Layout();
  EXPECT_VEC_EQ(box.GetSize(), glm::ivec2(400, 400));
  EXPECT_VEC_EQ(box.Child(0)->GetPosition(),
                glm::vec3(-150.F, 150.0F, Rectangle::LAYER_Z_INCREMENT));
  EXPECT_VEC_EQ(child2->GetPosition(),
                glm::vec3(-150.F, 50.0F, Rectangle::LAYER_Z_INCREMENT));
  EXPECT_VEC_EQ(child3->GetPosition(),
                glm::vec3(-150.F, -50.0F, Rectangle::LAYER_Z_INCREMENT));

  box.SetItemAlignment(layout::Anchor::HorizontalAlignments::Right);
  box.Layout();
  EXPECT_VEC_EQ(box.GetSize(), glm::ivec2(400, 400));
  EXPECT_VEC_EQ(box.Child(0)->GetPosition(),
                glm::vec3(150.F, 150.0F, Rectangle::LAYER_Z_INCREMENT));
...
TEST_F(VBoxTest, LayoutWithPadding) {
  ...
  box.Layout();
  EXPECT_VEC_EQ(box.GetChildrenSize(), glm::ivec2(100, 200));
  EXPECT_VEC_EQ(box.Child(0)->GetPosition(),
                glm::vec3(0.F, 140.0F, Rectangle::LAYER_Z_INCREMENT));
  ...
  EXPECT_VEC_EQ(box.GetChildrenSize(), glm::ivec2(100, 200));
  EXPECT_VEC_EQ(box.Child(0)->GetPosition(),
                glm::vec3(-145.F, 140.0F, Rectangle::LAYER_Z_INCREMENT));
  EXPECT_VEC_EQ(box.Child(1)->GetPosition(),
                glm::vec3(-145.F, 40.0F, Rectangle::LAYER_Z_INCREMENT));

  box.SetItemAlignment(layout::Anchor::HorizontalAlignments::Right);
  box.Layout();
  EXPECT_VEC_EQ(box.GetChildrenSize(), glm::ivec2(100, 200));
  EXPECT_VEC_EQ(box.Child(0)->GetPosition(),
                glm::vec3(135.F, 140.0F, Rectangle::LAYER_Z_INCREMENT));
  EXPECT_VEC_EQ(box.Child(1)->GetPosition(),
                glm::vec3(135.F, 40.0F, Rectangle::LAYER_Z_INCREMENT));
}
