...
namespace soil::stage::scene::gui {
    ...
    TEST_F(RectangleTest, Contruct) {
        ...
        EXPECT_VEC_EQ(rect.GetPosition(), glm::vec3(0.F, 0.F, Rectangle::LAYER_Z_INCREMENT));
    ...
    TEST_F(RectangleTest, UpdateScissor) {
        ...
        rect->SetPosition(glm::vec3(-400.F, -300.F, 0.F)); // center(0, 0) -> only 200px visible
        ...
        rect->SetPosition(glm::vec3(-400.F, -100.F, 0.F)); // center(0, 200) -> 400px of height visible
        rect->Update();
        EXPECT_VEC_EQ(rect->GetScissorRect().LowerLeftPosition, glm::ivec2(0, 0))
        EXPECT_VEC_EQ(rect->GetScissorRect().Size, glm::ivec2(200, 400))

        rect->SetPosition(glm::vec3(-200.F, -100.F, 0.F)); // center(200, 200) -> 400px visible
        rect->Update();
        EXPECT_VEC_EQ(rect->GetScissorRect().LowerLeftPosition, glm::ivec2(0, 0))
        EXPECT_VEC_EQ(rect->GetScissorRect().Size, glm::ivec2(400, 400))

        rect->SetPosition(glm::vec3(100.F, 200.F, 0.F)); // center(400, 400) -> 400px width, 350px height visible
        rect->Update();
    ...
    TEST_F(RectangleTest, UpdateScissorWithPadding) {
        ...
        EXPECT_VEC_EQ(rect->GetScissorRect().LowerLeftPosition, glm::ivec2(0, 0))
        EXPECT_VEC_EQ(rect->GetScissorRect().Size, glm::ivec2(400, 400))
        EXPECT_VEC_EQ(childRect->GetScissorRect().LowerLeftPosition, glm::ivec2(150, 150))
        ...
        rect->Update();
        EXPECT_VEC_EQ(childRect->GetScissorRect().LowerLeftPosition, glm::ivec2(200, 150))
        EXPECT_VEC_EQ(childRect->GetScissorRect().Size, glm::ivec2(50, 50))

        rect->SetPadding(glm::vec4(0, 0, 200, 200));
        rect->Update();
        EXPECT_VEC_EQ(childRect->GetScissorRect().LowerLeftPosition, glm::ivec2(150, 200))
        EXPECT_VEC_EQ(childRect->GetScissorRect().Size, glm::ivec2(50, 50))
    }
