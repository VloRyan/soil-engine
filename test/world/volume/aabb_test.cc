...
namespace soil::world::volume {
    ...
    TEST_F(AABBTest, IsInside) {
        ...
        ASSERT_TRUE(aabb.IsInside({-5.F, -5.F, -5.F}, {5.F, 5.F, 5.F})); // completely inside
        ASSERT_FALSE(aabb.IsInside({-10.F, -10.F, -10.F}, {-5.F, -5.F, -5.F})); // completely outside
        ASSERT_TRUE(aabb.IsInside({0.F, 0.F, 0.F}, {2.4F, 2.4F, 2.4F})); // partly inside
    }

    TEST_F(AABBTest, IsInsideXZ) {
        const auto aabb = AABB({1.F, 1.F, 1.F}); // -2.5, -2.5, -2.5 -> 2.5, 2.5, 2.5

        ASSERT_TRUE(aabb.IsInsideXZ({-5.F, 0.F, -5.F}, {5.F, 0.F, 5.F})); // completely inside
        ASSERT_FALSE(aabb.IsInsideXZ({-10.F, 0.F, -10.F}, {-5.F, 0.F, -5.F})); // completely outside
        ASSERT_TRUE(aabb.IsInsideXZ({0.F, 0.F, 0.F}, {2.4F, 0.F, 2.4F})); // partly inside
    }
