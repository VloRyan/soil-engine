...
namespace soil::world::volume {
    ...
    protected:
        ...
        static std::vector<const Volume*> VolumesAt(const QuadTree& quadTree, const std::int16_t index) {
            std::vector<const Volume*> vol;
            quadTree.GetVolumes(index, vol);
            return vol;
        }

        static std::vector<const Volume*> VolumesAt(const QuadTree& quadTree, const glm::vec3 point) {
            std::vector<const Volume*> vol;
    ...
    TEST_F(QuadTreeTest, InsertFourItems) {
        ...
        EXPECT_EQ(quadTree.GetNodeCount(), 1);
        EXPECT_THAT(VolumesAt(quadTree, 0), testing::ElementsAre(topLeft, topRight, bottomRight, bottomLeft));
    }
